...
async def access_token() -> str:
    return create_access_token("test3user@packt.com")
...
async def mock_event() -> Event:
    new_event = Event(
        ...
        descripttion="We will be discussing the contents of the FastAPI book in this event. \
...
async def test_update_event(default_client: httpx.AsyncClient, mock_event: Event,
    ...
    assert response.status_code == 200
    assert response.json()["title"] == test_payload["title"] #  "This test should fail"

@pytest.mark.asyncio
async def test_delete_event(default_client: httpx.AsyncClient, mock_event: Event,
                            access_token: str) -> None:
    test_response = {
...
@pytest.mark.asyncio
async def test_get_event_again(default_client: httpx.AsyncClient, mock_event:
                               Event) -> None:
    url = f"/event/{str(mock_event.id)}"
    response = await default_client.get(url)
